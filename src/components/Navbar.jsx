            ...
            {currentUser && isDashboardRoute && (
              <div className="relative" ref={actionMenuRef}>
                ...
                {mobileActionMenuOpen && (
                  <>
                    {/* Fixed backdrop preventing clicks & touch-scroll from passing to the background page */}
                    <div 
                      className="fixed inset-0 z-40 bg-navy-950/70 backdrop-blur-sm transition-opacity"
                      onClick={() => setMobileActionMenuOpen(false)}
                      onTouchMove={(e) => e.preventDefault()}
                      aria-hidden="true"
                    />

                    {/* Scroll-contained, height-constrained mobile menu container */}
                    <div 
                      className="absolute right-0 top-[calc(100%+0.75rem)] z-50 w-[min(20rem,calc(100vw-1.5rem))] max-h-[min(82vh,calc(100dvh-5.5rem))] flex flex-col rounded-2xl border border-violet-400/25 bg-[#130d22]/98 p-0 shadow-[0_18px_50px_rgba(0,0,0,0.65),0_0_28px_rgba(99,102,241,0.2)] backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden"
                    >
                      {/* Fixed Top Header inside Menu */}
                      <div className="flex items-center justify-between px-3.5 py-3 border-b border-white/10 shrink-0 bg-[#130d22]">
                        <div>
                      ...
                      {/* Smoothly Scrollable Options Area (Only options scroll, background screen stays static) */}
                      <div 
                        className="p-3 space-y-3 overflow-y-auto overscroll-contain flex-1 custom-scrollbar"
                      ...
                      >
                        ...
                        <div className="border-t border-white/10 pt-3 space-y-1.5">
                          ...
                          <button 
                            ...
                            className="flex items-center gap-3 w-full rounded-xl px-2.5 py-2.5 text-xs font-bold text-emerald-300 bg-emerald-500/10 border border-emerald-400/20 hover:bg-emerald-500/15 cursor-pointer"
                          >
                            <Headphones className="w-4 h-4" />
                            <span className="flex-1 text-left">Help & Student Support</span>
                            <span className="text-[9px] font-black uppercase bg-emerald-500 text-navy-950 px-1.5 py-0.5 rounded">24/7</span>
                          </button>
                          <div className="flex items-center gap-3 w-full rounded-xl px-2.5 py-2 text-xs font-semibold text-gold-300/70 border border-gold-500/10">
                            <Crown className="w-4 h-4" />
                            <span className="flex-1">Premium</span>
                            <span className="text-[9px] font-bold uppercase bg-gold-500/15 px-1.5 py-0.5 rounded">Coming Soon</span>
                          </div>
                        </div>
                      </div>
                    </div>
